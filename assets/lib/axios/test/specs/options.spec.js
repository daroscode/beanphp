describe('options', function () {
  ...
  it('should accept base URL', function (done) {
    ...
    getAjaxRequest().then(function (request) {
      expect(request.url).toBe('https://test.com/foo');
      done();
    });
  });
  ...
  it('should ignore base URL if request URL is absolute', function (done) {
    ...
    instance.get('https://someotherurl.com/');

    getAjaxRequest().then(function (request) {
      expect(request.url).toBe('https://someotherurl.com/');
      done();
    });
  });
  ...
  it('should change only the baseURL of the specified instance', function() {
    ...
    instance1.defaults.baseURL = 'https://instance1.example.com/';

    expect(instance2.defaults.baseURL).not.toBe('https://instance1.example.com/');
  });
